...
# Uses same fill+stroke approach as visualize_mr_with_uscrs but with white fill
visualize_mr <- function(match_id, match_data = heart_only_match_run_with_uscrs, show_blood_type = FALSE){
  ...
  # Create aesthetic mapping based on show_blood_type parameter
  if(show_blood_type) {
    # Show blood type as shape
    p <- match_data %>%
      filter(MATCH_ID == match_id) %>%
      ggplot(aes(x = PTR_SEQUENCE_NUM, y = PTR_DISTANCE, 
                 color = status, shape = CAN_ABO)) +
      geom_point(size = 4, alpha = 0.8, fill = "white", stroke = 1.5) + 
    ...
    # No blood type, use circle with fill+stroke
    p <- match_data %>%
      filter(MATCH_ID == match_id) %>%
      ggplot(aes(x = PTR_SEQUENCE_NUM, y = PTR_DISTANCE, 
                 color = status)) +
      geom_point(shape = 21, size = 4, alpha = 0.8, fill = "white", stroke = 1.5) + 
      scale_color_manual(values = c("Status 1" = "#d73027",                    # Most urgent - red
                                    "Status 1A (pediatric)" = "#d73027",       # Most urgent - red  
                                    "Status 1B (pediatric)" = "#f46d43",       # Very urgent - orange-red
                                    "Status 2" = "#fdae61",                     # High urgent - orange
                         ...
                         name = "Heart Status") +
      labs(x = "Sequence Number", y = "Distance (NM)", 
           color = "Heart Status", linetype = "Recipient")
  }
...
**Implementation**: Both `visualize_mr()` and `visualize_mr_with_uscrs()` functions updated with Version 4 approach.
