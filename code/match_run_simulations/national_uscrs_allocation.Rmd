...
heart_only_match_run_with_uscrs %>%
  ...
  geom_histogram(breaks = seq(0,50), alpha = 0.7, color = "white", size = 0.1)
```
...
heart_only_match_run_national %>%
  ...
  filter(sequence_position <= 300) %>%  # Focus on top 100 candidates for clarity
  ...
  summarise(
    ...
    sd_us_crs = sd(us_crs_score, na.rm = TRUE),
    n_candidates = n(),
    se_us_crs = sd_us_crs / sqrt(n_candidates),
    ci_lower = mean_us_crs - (1.96 * se_us_crs),
    ci_upper = mean_us_crs + (1.96 * se_us_crs),
    .groups = "drop"
  ) %>%
  ggplot(aes(x = sequence_position, y = mean_us_crs, color = allocation_system, fill = allocation_system)) +
