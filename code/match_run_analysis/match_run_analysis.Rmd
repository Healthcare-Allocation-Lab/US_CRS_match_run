...
sequence_comparison = sequence_comparison %>%
  mutate(
    durable_LVAD = ifelse(is.na(durable_LVAD), 0, durable_LVAD),
    durable_lvad_chr = ifelse(durable_LVAD == 0, "No", "Yes")
  )
...
```{r}
# Visualize match run for a specific match ID colored by US-CRS score
visualize_match_run_lvad <- function(match_id, 
                                policy = "acuity-us-crs",
                                match_data = sequence_comparison,
                                    show_us_crs = TRUE,
                                    sequence_limit = 300) {
  ...
  # Greyscale for US-CRS, ordered colors for status urgency
    if (show_us_crs){
      p <- match_data %>% filter(MATCH_ID == match_id) %>%
        ggplot(aes(x = sequence, y = PTR_DISTANCE, 
               fill = us_crs_score, color = durable_lvad_chr)) +
            scale_fill_gradient(low = "white", 
                                high = "black", na.value = "grey90", 
                        name = "US-CRS Score")
    } else{
      p <- match_data %>% filter(MATCH_ID == match_id) %>%
        ggplot(aes(x = sequence, y = PTR_DISTANCE, 
               color = durable_lvad_chr))
    }
  
  p <- p + geom_point(shape = 21, size = 2, alpha = 0.8, stroke = 1) + 
    labs(x = "Sequence Number", y = "Distance (NM)", 
         fill = "US-CRS Score", color = "Durable LVAD", linetype = "Recipient", shape = "Candidate blood type") +
    ggtitle(paste0("Match ID: ", match_id, " | Donor: ", donor_id, " (", donor_blood_type, ") | ", outcome)) +
    theme_minimal()
...
sequence_comparison %>% select(durable_lvad_chr, durable_LVAD)
...
# Visualize a specific match run with US-CRS scores
sample_match_id <- heart_only_match_run_counterfactual$MATCH_ID[2]  
sample_match_id
sample_match_id <- 1440203
visualize_match_run_lvad(sample_match_id, policy = "current", show_us_crs = FALSE)

sequence_comparison %>% filter(MATCH_ID == 1418339)

heart_only_match_run_with_uscrs %>% count(durable_LVAD)

sequence_comparison %>% 
  group_by(MATCH_ID) %>%
  count(durable_LVAD) %>% 
  filter(durable_LVAD == 1 & n/sum(n) < 0.4 & any(durable_LVAD == 0))

sequence_comparison %>% filter(MATCH_ID == 1440203		) %>%
  count(durable_LVAD)

ggsave(here("results", "current_6_status_lvad.pdf"), width = 9, height = 7)


```
```{r}
visualize_match_run_lvad(sample_match_id, policy = "national-us-crs", show_us_crs = FALSE)
ggsave(here("results", "national_uscrs_lvad.pdf"), width = 9, height = 7)
```

```{r}
visualize_match_run_lvad(sample_match_id, policy = "acuity-us-crs", show_us_crs = FALSE)
ggsave(here("results", "acuity_uscrs_lvad.pdf"), width = 9, height = 7)
...
ggsave(here("results", "us_crs_score_by_position.png"))
```
...
if(exists("sequence_comparison")) {
  ...
  # Create comprehensive lvad distribution visualization
  lvad_distribution_data <- sequence_comparison %>%
    filter(!is.na(current_sequence) & !is.na(national_sequence) & !is.na(acuity_sequence)) %>%
    pivot_longer(
      cols = c(current_sequence, national_sequence, acuity_sequence),
      names_to = "allocation_system",
    ...
    filter(sequence_position <= 100) %>%  # Focus on top 100 candidates for clarity
    group_by(allocation_system, sequence_position, durable_lvad_chr) %>%
    summarise(
      n_candidates = n(),
      .groups = "drop"
    )
  
  p_lvad_dist <- lvad_distribution_data %>%
    ggplot(aes(x = sequence_position, y = n_candidates, fill = durable_lvad_chr)) +
    geom_area(position = "fill", alpha = 0.8) +
    facet_wrap(~allocation_system, ncol = 1) +
    scale_fill_viridis_d(option = "turbo", direction = -1, 
                      name = "Durable LVAD") +
    scale_y_continuous(labels = scales::percent_format()) +
    labs(
      title = "Candidate lvad Distribution Across All Allocation Systems",
      x = "Sequence Position (Lower = Higher Priority)",
      y = "Proportion of Candidates"
    ) +
    theme_minimal() +
  ...
  # Save the comprehensive lvad distribution plot
  ggsave(here("results", "lvad_distribution_all_systems.png"), p_lvad_dist, 
         width = 14, height = 12, dpi = 300)
  
  # Summary statistics for lvad distribution
  lvad_dist_summary <- lvad_distribution_data %>%
    group_by(allocation_system, durable_lvad_chr) %>%
    summarise(
      total_in_top100 = sum(n_candidates),
      avg_position = round(weighted.mean(sequence_position, n_candidates), 1),
      .groups = "drop"
  ...
  heart_only_match_run_national %>%
    select(PX_ID, MATCH_ID, PTR_SEQUENCE_NUM, us_crs_score, national_uscrs_sequence, durable_lvad_chr) %>%
    rename(current_sequence = PTR_SEQUENCE_NUM) %>%
    pivot_longer(cols = c(current_sequence, national_uscrs_sequence), 
                 names_to = "allocation_system", 
                 values_to = "sequence_position") %>%
...
ggsave(here("results", "lvad_distribution_fallback.pdf"))
```
...
ggsave(here("results", "distance_analysis_by_sequence.pdf"), width = 12, height = 8, dpi = 300)
```
