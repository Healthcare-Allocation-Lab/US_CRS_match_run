...
```
# Adult patients with a missing US CRS
```{r}
...
## impute `us_crs` of 25 for adults with missing score
goal to remove this step after debugging
```{r}
heart_only_match_run_with_uscrs <- heart_only_match_run_with_uscrs %>%
  mutate(us_crs_score = ifelse(is.na(us_crs_score) & CAN_AGE_AT_LISTING>=18, 25, us_crs_score))

#check imputation successful
heart_only_match_run_with_uscrs %>%
  filter(CAN_AGE_AT_LISTING>=18 & is.na(us_crs_score)) %>%
  count(CAN_AGE_AT_LISTING)
```
