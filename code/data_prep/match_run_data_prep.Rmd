...
match_run <- read_sas("C:/Users/mwhite10/Downloads/ptr_hr_20220101_20221231_pub.sas7bdat")
tx_hr <- read_sas("C:/Users/mwhite10/Downloads/pubsaf2506/tx_hr.sas7bdat")
cand_thor <- read_sas("C:/Users/mwhite10/Downloads/pubsaf2506/cand_thor.sas7bdat")
donor_deceased <- read_sas("C:/Users/mwhite10/Downloads/pubsaf2506/donor_deceased.sas7bdat")
institution <- read_sas("C:/Users/mwhite10/Downloads/pubsaf2506/institution.sas7bdat")
institution
donor_disposition <- read_sas("C:/Users/mwhite10/Downloads/pubsaf2506/donor_disposition.sas7bdat")
donor_disposition %>% 
  filter(is.na(DON_TX_CTR_ID))
don_zips <- read_sas("C:/Users/mwhite10/Downloads/SupplementalData2506/donzip2506.sas7bdat")
match_run %>%
  count(MATCH_OPO_CTR_CD)

donor_disposition %>% group_by(DONOR_ID)
don_zips %>% filter(DON_HOME_ZIP != "")

institution %>% count(CTR_TY)
```
...
load("C:/Users/mwhite10/Downloads/pred_data.RData")

us_crs_data <- pred_data %>%
  select(PX_ID, current_date, prob_surv_6wk, durable_LVAD)
...
us_crs_mapper <- function(patient_id, match_date){
  ...
  lvad_value <- closest_score$durable_LVAD
  ...
  if(length(us_crs_score) > 0) {
    return(list(us_crs_score = as.numeric(us_crs_score[1]), prob_surv_6wk = prob_value, durable_LVAD = lvad_value))
  } else {
    return(list(us_crs_score = NA, prob_surv_6wk = prob_value, durable_LVAD = lvad_value))
  }
}
...
heart_only_match_run_with_uscrs <- heart_only_match_run %>%
  ...
  mutate(
    ...
    durable_LVAD = map_dbl(uscrs_results, ~ .x$durable_LVAD %||% NA_real_)
  ) %>%
